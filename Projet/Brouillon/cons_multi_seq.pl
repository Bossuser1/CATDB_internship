...
#	#-- pour les AGI:
#	$typId = "G";			# les id sont de type Gene 
#	my $requete = &SQLCHIPS::do_sql("select sg.spotted_sequence_id, sg.gene_name from $ppty.spot_gene sg, 
#  								$ppty.spotted_sequence ss where sg.spotted_sequence_id=ss.spotted_sequence_id and 
#  								type_feat in ($typefeat) and sg.GENE_NAME in ($listidents)");
#	$nbline = $requete->{NB_LINE};
#	if ( $nbline == 0 ) {
#		#-- pour les CATMA:
#		$typId = "P"; 			# les id sont de type Probe 
#		$requete = &SQLCHIPS::do_sql("Select spotted_sequence_id, seq_name from $ppty.spotted_sequence 
#    							where type_feat in ($typefeat) and SEQ_NAME in ($listidents)" );
#		$nbline = $requete->{NB_LINE};
#	}
	#--pour les AGI et les CATMA ensembles :
	my $requete = &SQLCHIPS::do_sql("select ss.spotted_sequence_id, sg.gene_name, ss.seq_name from 
							$ppty.spot_gene sg, $ppty.spotted_sequence ss where sg.spotted_sequence_id=ss.spotted_sequence_id 
							and ss.type_feat in ($typefeat) and (sg.GENE_NAME in ($listidents) or ss.SEQ_NAME in ($listidents))");
	$nbline = $requete->{NB_LINE};
...
#		my $reqdata = &SQLCHIPS::do_sql("Select s.spotted_seq_id, daf.replicat_id, daf.log_ratio, 
#    									daf.bonf_p_value from $ppty.diff_analysis_value daf, $ppty.spot s where 
#    									daf.spot_id = s.spot_id and s.spotted_seq_id in ($listspotseqId)");
		my $reqdata = &SQLCHIPS::do_sql("Select s.spotted_seq_id, daf.replicat_id, daf.log_ratio, 
    									daf.bonf_p_value from $ppty.diff_analysis_value daf, $ppty.spot s, $ppty.replicats r, 
    									$ppty.project p where daf.spot_id=s.spot_id and daf.replicat_id=r.replicat_id and  
    									r.project_id=p.project_id and p.is_public $public and s.spotted_seq_id in ($listspotseqId)");
...
#print "listrepId:\n",Dumper($listrepId),"\n";   ##test
		...
		my $reqinforep = &SQLCHIPS::do_sql("Select r.replicat_id, p.project_id, e.experiment_id, 
												r.replicat_extracts, p.project_name, e.experiment_name from $ppty.replicats r, 
												$ppty.project p, $ppty.experiment e where r.project_id = p.project_id and 
												r.experiment_id = e.experiment_id and p.is_public $public and r.replicat_id in 
												($listrepId) order by p.project_name, e.experiment_name, r.replicat_extracts");
		...
		my $reqseqgen = &SQLCHIPS::do_sql("select distinct ss.spotted_sequence_id, ss.SEQ_NAME, sg.GENE_NAME 
											from $ppty.spot s, $ppty.spotted_sequence ss, $ppty.spot_gene sg where ss.
											spotted_sequence_id = sg.spotted_sequence_id and ss.spotted_sequence_id = 
											s.spotted_seq_id and ss.spotted_sequence_id in ($lstspotseqId) and s.control_id in (0,2,3,6)");
		$nbline = $reqseqgen->{NB_LINE};
		for ( $j = 0 ; $j < $nbline ; $j++ ) {
#			$prob_name{ $reqseqgen->{LINES}[$j][0] } = $reqseqgen->{LINES}[$j][1];  # a supprimer
#			$gene_name{ $reqseqgen->{LINES}[$j][0] } = $reqseqgen->{LINES}[$j][2]; # a supprimer
...
"&nbsp;&nbsp;>> move mouse over the table to see information about projects, experiments, dye-swaps or values",
			$w->br,
			$w->i(
"&nbsp;&nbsp;cells of the first line delimit experiments, and are as large as the number of dye-swaps in the experiments is high"
			)
		), "\n", $donnees, "\n",
